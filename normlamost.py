...
matrix = np.loadtxt('waveflux8.txt')

duanwave  = matrix[:,0]
duanflux = matrix[:,1]

duanwave = duanwave[0:3860]
duanflux = duanflux[0:3860]
...
class Ransac:
    ...
    def ransac(self,samples, points_ratio = 0.01, epoch = 500, reject_dis = 8 ,inliers_ratio = 0.001):
...
normadata = [duanwave, guiyi]
npnormdata = np.array(normadata)
np.savetxt('npnormdata8.txt', npnormdata)
